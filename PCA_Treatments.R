...
group2<- "Full_CD8_wtIL2"
group3<- "Full_CD8_Mb_wtIL2"
# group4<- "fulldel_CD8_V7"
Trx_group<-c(group1, group2, group3)

group5<- "Full_CD8_V7"
group6<- "Full_CD8_monomer"
#group7<- "dualCAR_CD8_V7"
#group8<- "Full_CD4_control"
Ctrl_group<- c(group5, group6)
...
# make direction
dir_name <- "Treatments"#change back
...
my_shapes<- c(Full_CD8=22, Costimonly=21,CD3only=24, fulldel=25, dualCAR=7, Full_CD4=23)

pca_data$Construct<- factor(c(rep("Full_CD8", nrow(pca_data))))
pca_data$Treatment<- factor(c(rep("Rested", 4), rep("wtIL2", 4) , rep("Mb_wtIL2", 4) , rep("V7",4) , rep("monomer", 4)) )
pdf(file.path(dir_name, paste0("PCA4-5_",dir_name, ".pdf")), height = 5, width = 5)

ggplot(pca_data, aes(PC4, PC5)) + 
  geom_point(aes(fill = Treatment, shape = Construct), size = 3, stroke = 0.75, colour = "black", show.legend = T) +  # Black borders
  xlab(paste0("PC4 ", proportion_variance_explained$PC4, "%")) +
  ylab(paste0("PC5 ", proportion_variance_explained$PC5, "%")) +
