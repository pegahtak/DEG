...
group2<- "Full_CD4_control"
#group3<- "Full_CD8_control"
# group4<- "fulldel_CD8_V7"
Trx_group<-c(group1, group2)
...
pca_data$Construct<- factor(c(rep("Full_CD8", 4), rep("Full_CD4", 4), rep("Full_CD8", 4), rep("Full_CD4", 4)))
pca_data$Treatment<- factor(c(rep("Rested", 8), rep("V7", 8)) )
pdf(file.path(dir_name, paste0("PCA2-3_",dir_name, ".pdf")), height = 5, width = 5)
